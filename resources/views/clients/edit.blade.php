                    ...
                    <form method="POST" action="{{ route('clients.update', $client) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="direccion" id="direccion" rows="3"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('direccion') border-red-500 @enderror">{{ old('direccion', $client->direccion) }}</textarea>
                            @error('direccion')
                                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="canal_id" id="canal_id"
                                ...
                                @foreach($canales as $canal)
                                    <option value="{{ $canal->id }}" {{ old('canal_id', $client->canal_id) == $canal->id ? 'selected' : '' }}>
                                        {{ $canal->descripcion }}
                                    </option>
                                @endforeach
                            </select>
